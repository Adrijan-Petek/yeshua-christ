...
export default function FaithPage() {
  ...
  return (
    <div className="space-y-6 px-4">
      ...
      <section className="rounded-xl border border-stone-200 bg-white p-4 sm:p-5 shadow-sm dark:border-stone-700 dark:bg-black">
        <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
          ...
          <a
            ...
            className="inline-flex items-center justify-center rounded-lg border border-stone-200 bg-stone-50 px-4 py-2.5 text-sm font-medium shadow-sm hover:bg-stone-100 dark:border-stone-700 dark:bg-stone-800 dark:hover:bg-stone-700"
          >
            Post on Farcaster
          </a>
        </div>
      ...
      <section className="rounded-xl border border-stone-200 bg-white p-4 sm:p-5 shadow-sm dark:border-stone-700 dark:bg-black">
        <h2 className="mb-2 text-base sm:text-lg font-semibold">Community Rules</h2>
        <ul className="list-disc space-y-1 pl-5 text-xs sm:text-sm text-stone-700 dark:text-stone-300">
          {rules.split('\n').map((rule, i) => rule.trim() && <li key={i}>{rule}</li>)}
        </ul>
      </section>

      <section className="rounded-xl border border-stone-200 bg-white p-4 sm:p-5 shadow-sm dark:border-stone-800 dark:bg-stone-950">
        ...
        <div className="space-y-2.5">
          {casts.slice(0, 12).map((cast) => (
            <a
              ...
              target="_blank"
              rel="noopener noreferrer"
              className="block rounded-lg border border-stone-200 bg-stone-50 px-3.5 py-2.5 hover:bg-stone-100 dark:border-stone-800 dark:bg-stone-900 dark:hover:bg-stone-800"
            >
              ...
              <p className="whitespace-pre-wrap text-xs sm:text-sm text-stone-900 dark:text-stone-100">{cast.text || ""}</p>
